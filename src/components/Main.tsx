      ...
      <div style={{ marginBottom: '1rem' }}>
        <details>
          ...
          <p>
            ...
            {`), нужно набрать 100 тысяч подписей, максимум по ${N_PER_REGION_MAX} в каждом регионе России.`}
          </p>

          <p>{`По сведениям из официального Telegram канала, штаб хочет собрать ${TOTAL_DESIRED} подписей, c запасом, так чтобы, за вычетом испорченных подписей, набралось нужное количество - ${TOTAL_REQUIRED}. Таким образом, мы принимаем как гипотезу, что необходимо собрать по ${N_PER_REGION_DESIRED} в каждом регионе, вместо ${N_PER_REGION_MAX}.`}</p>
