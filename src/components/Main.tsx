      ...
      <div style={{ marginBottom: '1rem' }}>
        <details>
          ...
          <p>
            ...
            {`), кандидатy от непарламентской партии нужно набрать 100000 подписей, максимум по ${N_PER_REGION_MAX} в каждом регионе России.`}
          </p>
          ...
          <p>
            {`В суммарной таблице мы выделили крупным текстом общее количество подписей без учета излишков - т.е. принимая в рассчет только максимум ${N_PER_REGION_MAX} подписей на каждый регион ("цель минимум") или максимум ${N_PER_REGION_DESIRED} подписей на каждый регион ("цель желаемая"). Например, если в регионе собрано 10000 подписей, то мы в сумму "цели желаемой" мы включаем только ${N_PER_REGION_DESIRED} из них, а в сумму "цели желаемой" - ${N_PER_REGION_MAX}. Если в регионе собрано 1600, то в обе суммы идут все 1600. Если в регионе собрано между ${N_PER_REGION_DESIRED} и ${N_PER_REGION_MAX} подписей, то в таком случае в "цель минимум" идут только ${N_PER_REGION_MAX}, а в "цель желаемая" пойдут все подписи.`}
          </p>
          <p>
            {
          ...
          <p>
            {
              'В таблице регионов показано население каждого региона, а также количество собранных подаисей в пересчёте на 1 млн. человек. Данные о численности населения регионов взяты с '
            }
