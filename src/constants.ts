export const TITLE = 'Борис Надеждин 2024: неофициальный, но полезный сайт сделаный волонтёрами.';
export const DESCRIPTION =
  'Отслеживание сбора подписей за кандидата в презеденты Бориса Надеждина, cтатистика по регионам, адреса штабов где можно поставить подпись.';
export const IMAGE_URL = 'https://raw.githubusercontent.com/bnad24/bnad24.github.io/main/public/social.png';
export const HASHTAG = 'надеждин2024';

export const N_PER_REGION_MAX = 2_500;
export const TOTAL_REQUIRED = 100_000;
export const N_REGIONS_REQUIRED = TOTAL_REQUIRED / N_PER_REGION_MAX;

export const TOTAL_DESIRED = 150_000;
export const N_PER_REGION_DESIRED = TOTAL_DESIRED / N_REGIONS_REQUIRED;
